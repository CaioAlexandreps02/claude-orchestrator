...
12 achados reais ao todo, entre auditoria de código e testes ao vivo com o plugin instalado.
...
| 12 | `classify-prompt.py` (`log_routing_decision`) | Campo `savings` de cada sessão diária (dentro de `sessions[]`) nunca era incrementado — só o `estimated_savings` agregado no topo do arquivo era calculado certo. Resultado: `/router-stats` sempre mostrava `savings: 0.0` em todo dia individual, mesmo com economia real acontecendo | Caio pediu análise de gasto via `/router-stats` logo após atualizar pro 0.3.3; notei o campo zerado ao ler o JSON pra montar o relatório | Adicionada a linha que faltava (`session["savings"] += savings`, com `setdefault` pra sessões antigas sem o campo); testado comparando o arquivo antes/depois de rodar uma classificação real, confirmado que incrementa |
